...
- [docs/design/AgenticWorkflow.html](docs/design/AgenticWorkflow.html) — エージェンティック開発の運用フロー(主/配下サブエージェントの役割、停止点、レビューのラリー、メモリ規約)
...
- 作業が終わった時、私のことを「貴殿」と呼び。結果を簡潔に報告する。
- **コミット履歴を報告しない**: コミットの一覧/ハッシュ/件数/`git log` の結果などをチャット応答に含めない。コミットがあった事実を一言添えるのは可だが、ハッシュや差分の羅列は `git log` で見られるので不要。報告は「何が変わったか/何ができるようになったか」を中心にする。
