...
| OS | 状態 | 備考 |
|---|---|---|
| **Windows 11** | ✅ 完全対応(MVP の主ターゲット) | 全 backend が動く |
| macOS / Linux | ⚠ 部分動作 | 下記「Windows 専用機能」を別 backend に置き換える必要あり |

### Windows 専用の機能 / backend

| 機能 | 影響 | 他 OS での代替 |
|---|---|---|
| `sapi` TTS(MVP の既定 TTS) | macOS / Linux で読み上げ不可 | `piper`(ローカル)/ クラウド TTS / **TTS=「(なし)」**(字幕モード) |
| `proc-tap`(per-process キャプチャ) | macOS / Linux で「特定アプリの音だけ翻訳」不可 | 代替なし(配布方針: Windows 専用機能として明示) |
| スピーカループバック(`[LB] ...`) | OS ごとに仕組みが違う | macOS: BlackHole 等の仮想デバイス、Linux: PulseAudio / PipeWire monitor を別途用意 |

VAD / ASR / 翻訳 / Piper TTS / クラウド系 backend はクロス OS で動作します。
詳細な OS 別動作確認は [`docs/design/TaskList.md`](docs/design/TaskList.md) Phase 4 を参照。
