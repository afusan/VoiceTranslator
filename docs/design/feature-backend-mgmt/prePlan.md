# prePlan: レイヤーで使うバックエンドやパラメータの調整 — 実装方針

レイヤ内のバックエンド/モデル/パラメータの差し替え機能を実装する前の、論点ごとの
「問題」と「決定」を記録する作業用ドキュメント。

- 検討の議論詳細は [tmp/report11.md](../../../tmp/report11.md) を参照
- 本ブランチでの実装計画は [Plan.md](Plan.md) を参照

## 決定サマリ

| # | 論点 | 主な決定 |
|---|---|---|
| 1 | 認証情報の保管 | OS keychain + 平文ファイル(名前は `.env` 以外)の 2 段。新ステータス `MISSING_CREDENTIALS` 追加、認証 UI は各 `LayerSettingsDialog` 内、全レイヤ状態をテキストボックスに集約 |
| 2 | クラウド利用同意 | 初回モーダル + 常時バッジ。backend 単位で永続化、`suppress_dialogs` で一括 OFF も用意。キャンセル時は操作取り消し |
| 3 | 失敗時の挙動 | 直列特性に従い復帰不能ならパイプライン全停止。ネットワーク系テキスト API は 3 回リトライ、それ以外即停止。エラーは backend に直近 N 件保持→ステータス領域集約 |
| 4+5 | パラメータ公開 | パネルは backend 選択のみ。モデル選択は詳細ダイアログ内。preset 概念は導入せずモデル選択で兼ねる。個別パラメータは defaults + `config.yaml` 手編集 |
| 新 | ロード方式・リソース可視化 | 起動時 auto-load 既定 OFF、詳細に Load ボタン + Auto-load トグル + 目安リソース。開始ボタンは常時押せ未ロードはその場でロード。直近 5 件処理時間表示 |
| 6 | プロファイル自動調整 | **見送り**(可視化のみ実装、提案 UI は別機会) |
| 7 | `BackendCapabilities` 拡張 | `is_cloud` / `requires_credentials` / `service_name` / `terms_url` / `is_retryable_on_error` / 推奨モデル + リソース申告 を追加。コスト目安は不要 |
| 8 | テスト戦略 | 新 backend 追加時は `docs/forRunner/` の手順で検証、必要なら middle 追加。通常機能追加では実行しない、モデル追加等の限定的タイミングで手動実行 |
| 9 | 状態管理 | `MISSING_CREDENTIALS` 追加(論点 1)以外は既存仕組みで対応、追加事項なし |

---

## 論点 1: 認証情報(API key 等)の保管方針

**問題**: クラウド/有償 backend を追加すると API key の保管が必要になる。`config.yaml` 平文は GitHub 公開時の事故源で NG。

**決定**:
- **保管先**: 2 段フォールバック方式を採用
  - 第一: **OS keychain**(`keyring` ライブラリ経由)
    - Win = Credential Manager、macOS = Keychain Services、Linux = Secret Service(D-Bus)
    - いずれも **OS の API を直接叩く**(独自フォーマットの暗号ファイルを置くわけではない)ので、OS 間の信頼性差は実用上問題なし
  - 第二: **平文ファイル**(`.gitignore` 必須、開発/CI 用途)
    - **`.env` という名前は使わない** — web スキャナ系の bot が標的にしている(`/.env` への直叩き等)
    - 具体名は実装時に確定(候補: `local.secrets` / `app.secrets` / `secrets.local`)
- **key 不足 / 認証失敗時の挙動**(既存挙動を一部変更):
  - **新ステータスを追加**(仮称 `MISSING_CREDENTIALS`、既存 `NOT_DOWNLOADED` とは別物として区別)
  - 認証/ロード失敗時はこのステータスを表示
  - 該当ステータスがあるレイヤが 1 つでもあれば **「開始」ボタンをマスク**(`is_loaded` の判定に組み込む)
  - **認証情報の入力は各レイヤの `LayerSettingsDialog` 内で行う**(backend の `requires_credentials=True` フラグで入力フィールドを動的に表示)
- **全レイヤのステータス可視化**:
  - **ステータス表示用のテキストボックスを追加**(エラー文を含むサマリを 1 箇所に集約)
  - レイヤ状態が変わったら、各レイヤでエラーを取り直してテキストボックスを更新するイメージ

---

## 論点 2: クラウド backend 利用時の「同意」UX

**問題**: クラウド backend に切替えると音声/テキストが外部 API に送信される。ユーザに意識させずに切替えられるのはプライバシ事故源。バックエンドは「ローカル動作」と「クラウド動作」に明確に二分されるので、`BackendCapabilities.is_cloud: bool` で判別する。

**決定**:
- **方式**: (a) 初回モーダル + (b) 常時バッジ の併用
  - 初回モーダル: 該当 backend を初めて選んだとき同意ダイアログを出す
  - 常時バッジ: クラウド backend に ☁ アイコンを SettingsPanel のプルダウン項目・状態ラベル等で常時表示
- **同意の粒度**: backend 単位(`consents.<backend>: true`)
  - 例: `consents.openai_whisper_api: true` / `consents.deepl_api: true`
- **同意取り消し UI**: 当面は作らない(別 backend に切替えれば実害なし。要望が出てから対応)
- **「今後表示しない」master switch**: `config.yaml` に `consents.suppress_dialogs: true` を追加。有効時は全クラウド backend で同意ダイアログをスキップして即切替
- **同意ダイアログ文言**: 全 backend 共通テンプレートに placeholder 差し込み方式。送信先 / 送信データ種別 / 利用規約 URL を表示
  - `BackendCapabilities` に `service_name: str | None` と `terms_url: str | None` を追加し、ダイアログから参照
- **キャンセル時の挙動**:
  - 操作を**取り消し**(set_setting を呼ばない、ConfigStore は変化なし)
  - SettingsPanel のプルダウン表示値だけを元の値に戻す(視覚的に「変更されてない」を明示)
  - 同意確認は `set_setting` を呼ぶ**前**に挟む(後から呼ぶとロールバック処理が必要になる)
  - 初回起動で「previous」が無いケースは `DEFAULT_CONFIG` の初期値に戻す

---

## 論点 3: 失敗時のリトライ・停止・観測方針

**問題**: クラウド backend のタイムアウト/接続失敗/認証エラー、ローカル backend の I/O 失敗等への対処。パイプラインは直列なので**前段失敗で下流の処理が意味を成さない**前提あり。

**決定**:
- **パイプライン停止条件**: リトライで改善しない失敗が起きたら **パイプライン全体を停止**
  - 前段が失敗した発話を後段に流しても無意味
  - 1 発話だけ SKIP して継続、はやらない(直列パイプラインの特性に従う)
- **リトライ方針**(backend ごとに違う):
  - **ネットワーク経由のテキスト系 API**(クラウド翻訳・クラウド ASR 等): 3 回まで指数バックオフでリトライ → それでも失敗なら停止
  - **それ以外**(ローカル backend、デバイス I/O、認証失敗、ロード失敗 等): リトライせず即停止
  - 判定は backend 自身が `BackendCapabilities.is_retryable_on_error: bool` 等で申告する形が綺麗
  - 既存の `ErrorHandler` の severity(RECOVERABLE / FATAL / SKIP / WARN)とどう組み合わせるかは実装時に整理
- **観測性**(エラーをユーザに見せる経路):
  - **各 backend が直近 N 件のエラーを内部に保持**(N は暫定 5〜10)
  - backend に `get_recent_errors()` のような問い合わせ口を追加
  - UI(論点 1 で決めたステータステキストボックス)が各レイヤから取得して集約表示
  - 表示には**どのレイヤ・どの backend で何が起きたか**を必ず明示
  - エラーごとにダイアログ/ポップアップは出さない(うるさいので)。**ステータス領域への追記のみ**
- **ユーザ設定の自動変更**: しない(`config.yaml` は不変)
- **自動フォールバック**: MVP では実装しない
  - 「クラウド失敗 → 黙ってローカル」は信頼性・透明性の観点で原則やらない
  - 将来どうしても必要になったら `DEFAULT_CONFIG.backends.<layer>` の初期値固定をフォールバック先とする(ユーザ設定済みの previous でも一時的な runtime override でもなく、配布既定値)
  - 当面はステータス領域からユーザが手動で別 backend に切り替える運用

---

## 論点 4 + 5: パラメータ公開とモデル切替 — 詳細ダイアログに集約

**問題**: 個別パラメータ調整は <5% のユーザにしか必要ないが、UI に全部出すと過剰。一方で全く触れないと degenerate のような不具合に対処できない。実行環境のスペックは PC によって大きく差があり、ユーザに細かい個別調整を強いるのは現実的でない。

**決定**:
- **SettingsPanel(常時表示)**: backend 選択のみ。**モデル選択もパラメータ調整もここには出さない**
- **詳細ボタン → `LayerSettingsDialog`** に集約:
  - モデル選択(ドロップダウン)
  - 将来のパラメータ調整も同じダイアログ内に同居予定
- **専用の preset 概念は導入しない**(fast/balanced/accurate のような抽象 preset は作らない)
  - モデル選択そのものが「速度↔精度トレードオフ」の主軸として十分
- **個別パラメータ調整は当面コード defaults + `config.yaml` 手編集**(GUI 化は後回し)
  - 妥当な defaults を持つのは backend 開発者の責任(degenerate 修正で得た教訓)
- **各 backend が UI に出すモデル一覧を申告**: 全モデル列挙ではなく 3〜5 個に絞る

---

## 論点 (新): モデルロード方式とリソース可視化

**問題**: モデル選択肢が増えると起動時に全部ロードするのは重い。マシンスペックに見合わない選択を防ぎたい。動作中の負荷感もユーザに見せたい。

**決定**:
- **起動時の自動ロードは既定で OFF**(現状の load_models_async を無条件発火する挙動は廃止)
- **詳細ダイアログ内に追加する UI**:
  - **[Load Model] ボタン**: 手動でロード実行
  - **[Auto-load: ON/OFF] トグル**: 起動時 / backend 変更時の自動ロード有効化(既定 OFF)
  - **目安リソース**: 「RAM 2GB / VRAM 1GB」のような注記表示
  - **直近 5 件の処理時間平均 + 目安時間**: 既存 `processtime.csv` の `derive_stage_durations` を流用、メモリ内に直近 N 件保持(CSV 再読みしない)
- **リソース申告の仕組み**: backend が各モデルに `{ram_gb, vram_gb_if_gpu}` を宣言。検出した hw(RAM 量・GPU 有無・VRAM 量)と比較
  - CPU 性能差は判定材料に入れない(同じ CPU 名でも世代で性能差大、雑な推測より profile 補正に任せる)
- **明らかな容量不足は警告ダイアログ**:
  - 判定基準(暫定): 必要 VRAM > 利用可能 VRAM × 0.9、または 必要 RAM > 利用可能 RAM × 0.9
  - 「動かないかもしれません。続行しますか?」を確認、強制はしない
- **開始ボタンの挙動変更**:
  - **常に押せる**(現行の「全 LOADED 前提で gate」を廃止)
  - 未ロード layer があれば**ロードを実行 → 完了後にパイプライン開始**
  - ロード中の UI は「Loading...」を表示、capture/pipeline はロード完了まで待機
  - 例外: 論点 1 の `MISSING_CREDENTIALS` で gate するのは継続(認証不足はロードしても無意味)

---

## 論点 6: プロファイル駆動の自動調整提案 — 見送り

**問題**: 計測したプロファイルから「このマシンには別モデルがおすすめ」を提案する UI の検討。

**決定**: **MVP では実装しない**。
- 自動調整は予測困難な挙動変更を生むため、慎重に設計したい(暗黙の挙動変更はサポート騒ぎになる)
- まずは可視化(直近平均と目安時間の表示)で十分(可視化部分は「ロード方式・リソース可視化」セクションに含めて実装)
- 実装するとしても別ブランチで腰を据えて(プロファイル指標の閾値、提案文言、頻度、自動 vs オプトイン 等を別途整理)

---

## 論点 7: `BackendCapabilities` の拡張

**問題**: クラウド/有償 backend を判別したり、UI でバッジを出したり、エラー時のリトライ可否を判定するための capability 情報が現状不足。

**決定**: 以下のフィールドを `BackendCapabilities` に追加する(既存の `supported_languages` / `requires_gpu` / `notes` は維持):

| 追加フィールド | 用途 | 出元論点 |
|---|---|---|
| `is_cloud: bool` | ☁ バッジ表示、同意ダイアログのトリガ | 論点 2 |
| `requires_credentials: bool` | 認証入力 UI の動的表示、key 未設定検知 | 論点 1 |
| `service_name: str \| None` | 同意ダイアログ内の送信先表示 | 論点 2 |
| `terms_url: str \| None` | 同意ダイアログ内の利用規約リンク | 論点 2 |
| `is_retryable_on_error: bool` | 失敗時にリトライするかの判定 | 論点 3 |
| 推奨モデル一覧 + 各モデルの `{ram_gb, vram_gb_if_gpu}` + 目安処理時間 | UI 表示、リソース判定、目安時間表示 | 論点 4-5 / 新 |

**保留**: コスト目安(`estimated_cost_per_min` 等)は追加しない。正確な算出が難しく、ユーザの実感とずれやすい。要望が出てから検討。

---

## 論点 8: 新規 backend 追加時のテスト戦略

**問題**: backend が増えるたびに middle テストが線形に増え、開発者の検証時間が爆発する。

**決定**:
- **新規 backend 追加時は `docs/forRunner/` の検証手順を使う**(`runner_asr` / `runner_translator` / `runner_pipeline` 等で実モデル走行)
- 必要なら `tests/` 配下に **middle テストとして追加**(`@pytest.mark.middle`)
- ただし **通常の機能追加時(コード変更だけの PR 等)は middle を実行しない** — 既存通り small のみ
- **モデル追加・パラメータ調整・性能検証等の使用時期を限定**して、手動で middle を実行する運用
- 各 backend を個別実行可能にするため `@pytest.mark.backend_<name>` のようなマーカーを併用(必要に応じて)

---

## 論点 9: 切替・状態管理(確認)

**問題**: backend 切替時の状態遷移、認証失敗 / ロード失敗の検知。

**決定**(これまでの論点でほぼ決定済み、ここでは確認のみ):
- 新ステータス `MISSING_CREDENTIALS` を追加(論点 1 で決定)
- backend 切替の状態遷移は既存パターンを継続(`set_setting` → キャッシュ pop → 再ロード)
- ロード失敗時のエラーは backend 内に保持しステータス領域に集約(論点 3 で決定)
- **追加事項なし** — これまでの論点で必要な変更は出尽くしている
