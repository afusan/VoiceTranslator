...
### R-5. `keyring` の依存と CI / 開発環境 [解消 2026-05-29]
- **問題**: keyring は OS の secret service を要求。Linux で headless / SSH / Docker では失敗する。テストで実 keyring に触ると開発者の環境を汚染する
- **解消方針**: 用途別に 3 層で使い分け
  - **テスト**: `keyring.set_keyring(backend)` で **公式の test double** を注入
    - `InMemoryKeyring`(KeyringBackend サブクラス、dict 保管)で正常系
    - `keyring.backends.fail.Keyring` で「keyring 使用不可」シナリオを再現し fallback 経路を検証
    - `tests/_fixtures.py` に `fake_keyring` / `failing_keyring` の fixture を用意
    - `credentials.py` の wrapper 関数を monkeypatch する案より idiomatic(我々の fallback ロジック自体をテストできる)
  - **開発者ローカル(実 API 動作確認用)**: **平文ファイル(`.env` 代替)を使う**
    - 開発者は秘密の API key を keyring に入れず、プロジェクト直下の secrets ファイル(名前は実装時確定、`.gitignore` 必須)に書く
    - 「テスト時は keyring もファイルも触らない」「実機検証時はファイルを使う」「エンドユーザは keyring」のすみ分け
  - **エンドユーザ配布版**: 第一 keyring、不可なら平文ファイル fallback(prePlan 通り)
- **依存**: `keyring` を `pyproject.toml` の通常依存に追加(optional にしない)。サイズが小さく必須化のコストが低い
- **fallback の起動時検知**: 起動時に `keyring.get_password(test_service, test_user)` を 1 回試して、不可なら以降は平文ファイル一択。判定結果は `app.log` に WARN として記録(ユーザへのアラームは出さない)
- **CI への影響**: リモート CI 不要方針(CLAUDE.md)なので、CI 環境固有の対応は不要。手元の pytest が in-memory fake で完結すれば十分
