...
代わりに pycaw の `IAudioMeterInformation.GetPeakValue()` を永続 COM ワーカスレッド内の
5fps poll で読み、最新値を atomic float に保持する(`_PeakWorker` 方式 / 段階 3-fix):

```
[ダイアログ]
  PID 選択 → ▶ 試聴開始 押下
  → process_enumerator.start_audition(pid) を呼ぶ
       ↓ (ワーカに対象 PID を設定)
    [_PeakWorker (永続スレッド)]
       ループ: 200ms 毎に meter.GetPeakValue() を取って _latest_peak に保持
  → GUI スレッドの after(100ms) で pe.latest_peak() を atomic 読み
...
- **永続スレッド方式の利点**: GUI スレッドと COM スレッドモードの競合(STA / MTA)を回避、
  毎秒スレッド生成 0 個、CoInitialize は 1 回だけ
