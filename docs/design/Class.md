...
|---|---|---|---|
| `AudioCaptureBackend` | `start(id)` / `read_chunk(timeout) -> PcmChunk` / `stop()` / `capture_kind() -> CaptureKind`(classmethod) | `SoundcardCaptureBackend` | Win/Mac/Linux 抽象化。`capture_kind` で「デバイス単位」(`DEVICE`、既定)/「プロセス単位」(`PROCESS`)を宣言。SettingsPanel が `CaptureKind` 主体のプルダウン表記に使う(ProcTap 取り込み 段階 1 で導入)。 |
