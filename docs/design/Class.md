...
| `BackendCatalog` | **backend クラスのメタ情報問合せ口(状態なし)**(refactor-ui-3move P3 で AppController から分離)。capture_kind / 対応言語 / credential_spec / capability hint を**インスタンス化せずに**引く。未登録・例外時は安全側の既定値に縮退(GUI の防御縮退が依存する規約)。`AppController.catalog` で公開。 |
| `CredentialsService` | **認証情報の保管・疎通確認・verified フラグ管理**(P3 で AppController から分離)。CredentialsStore(lazy 初期化)+ ConfigStore の `credentials.*` に閉じる。`verify_and_save` は成功時のみ保存 + verified=True。backend キャッシュには触らない(認証成功後の reload はランタイム側)。`AppController.credentials` で公開。 |
...
| `get_status_snapshot() -> tuple[list[LayerStatusLine], list[tuple[LayerKind, ErrorRecord]]]` | 全レイヤの状態 + 直近エラーを**整形前のデータ**で返す(Phase C3 改 / refactor-ui-3move P1)。文字列への整形は UI 側 `gui/logic/status_summary.py` の役割(旧 `get_status_summary` は整形ごと UI へ移動)。 |
| `catalog` / `credentials` プロパティ | **P3(refactor-ui-3move)**: メタ問合せの実体 `BackendCatalog` と認証フローの実体 `CredentialsService` を公開する。新規コードはこちらを直接使う。 |
| メタ問合せ / 認証の互換窓(`get_capture_kind` / `get_supported_*_languages` / `supports_auto_detect` / `get_credential_spec` / `get_backend_capability_hint` / `get_credential` / `set_credential` / `delete_credential` / `has_credential` / `is_backend_verified` / `invalidate_verification`) | P3 で実装本体を `BackendCatalog` / `CredentialsService` に移管した **1 行委譲**。既存 GUI / テストの呼び出し互換のために残置(参照の全付け替え + 削除は P4 任意)。 |
| `verify_and_save_credentials(layer, name, values) -> VerifyResult` | `CredentialsService.verify_and_save`(疎通確認 + 保存 + verified 永続化)に委譲した上で、**Phase F1 の後処理**(認証成功時、該当レイヤが MISSING_CREDENTIALS なら reload)を行う。backend キャッシュに触るためこの後処理だけはランタイムの責務。 |
...
- 開始ボタンは常時押下可(Phase B)。disable になるのは `MISSING_CREDENTIALS` / `DOWNLOADING` /
