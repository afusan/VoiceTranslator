...
| `get_status_snapshot() -> tuple[list[LayerStatusLine], list[tuple[LayerKind, ErrorRecord]]]` | 全レイヤの状態 + 直近エラーを**整形前のデータ**で返す。文字列への整形は UI 側 `gui/logic/status_summary.py` の役割。 |
