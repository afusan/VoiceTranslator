...
| `TtsBackend` | `synthesize(text, tgt_lang) -> (pcm, samplerate)` + `supported_output_languages()` | `SapiTtsBackend` | pyttsx3経由、WAV経由でPCM取得。対応読み上げ言語は classmethod で宣言(SAPI は Windows 標準 voice 前提で `["ja", "en"]` 固定)。UI 側で「Translator 出力言語が TTS で読めない」なら警告バナーを出すが、ユーザ選択は変更しない。 |
