...
| `TranslatorBackend` | `translate(src_text, src_lang, tgt_lang) -> str` + `supported_target_languages()` / `supported_source_languages()` | `Nllb200TranslatorBackend` | 200 言語対応、ローカル動作。対応言語は ISO 639-1 で宣言(クラスメソッド、未ロードで問い合わせ可)。対称な backend なら source は default(target と同じ)、非対称(片方向)ならオーバーライド。 |
