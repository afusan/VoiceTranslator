...
| `AsrBackend` | `transcribe(pcm, hint) -> (text, lang)` + `supported_input_languages()` / `supports_auto_detect()` | `FasterWhisperAsrBackend` / `OpenAiWhisperAsrBackend` | `task=transcribe` 固定。対応言語は backend ごとに宣言(Whisper 系は `common/whisper_languages.py` の 99 言語を共有) |
