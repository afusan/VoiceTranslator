...
- 関数名は歴史的経緯で `_active_` を含むが、実体は Active + Inactive(コメントで明示)
- `_list_active_sessions` 内のフィルタ呼び出しを `_is_capturable` に差し替え
- `runner_proc_list`(診断ツール)の出力メッセージも更新
...
ユーザ側で `py -m uv pip install pycaw==20240210` でダウングレードして使う。
