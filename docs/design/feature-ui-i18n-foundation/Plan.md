...
- **Phase 4b(完了)**: 辞書追加(翻訳語彙)+ カタログ間整合検査。
  - `_EN` / `_ZH` / `_ES`(各 218 キー)を追加。`available_locales()` に ja / en / zh / es が
    出て、停止中に切替可能。`_LOCALE_DISPLAY_NAMES` で各言語自身の表記(English / 中文 / Español)。
  - カタログ間整合検査(全ロケールのキー集合一致 + placeholder 名一致 + 全ロケールでの
    テンプレート描画)を `test_i18n.py` に追加。
  - **翻訳品質は機械翻訳で未レビュー**(約束語・モデル名・記号は原語維持)。ネイティブ確認は
    別途。これ以上の言語追加も `_CATALOGS` に辞書を足すだけ(整合検査が漏れを担保)。
