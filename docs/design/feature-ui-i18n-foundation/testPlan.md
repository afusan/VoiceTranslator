...
## 2. キー健全性検査(AST ベース・辞書の健全性)
- ja 辞書に重複キーが無い。
- **AST 解析で `tr("...")` のリテラルキーを全抽出し、ja 辞書と突合する**:
  - (a) コードで使うが辞書に無いキー(欠落)= 0 件であること。
  - (b) 辞書にあるがコードで未使用のキー(死にキー)= 0 件であること。
- 動的キー(`tr(f"...")` や変数キー)が混入していないこと(規約違反の検出)。
  混入時は検査が成立しないため、テストで弾く。
