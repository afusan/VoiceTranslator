...
py -m uv run --extra cuda python -m voice_translator.dev.runner_asr `
    ...
    --model medium --beam-size 5 --lang-hint eng
```
...
| `--lang-hint` / `-l` | `auto` | 言語ヒント(ISO 639-3、例: `eng`)。`auto` で自動検出 |
...
py -m uv run --extra cuda python -m voice_translator.dev.runner_translator `
    --input logs/dumps/20260528-160000-0001/seq_0042_asr.json --tgt-lang jpn
...
# stdin から
"good morning" | py -m uv run --extra cuda python -m voice_translator.dev.runner_translator --src-lang eng --tgt-lang jpn
```
...
| `--output` / `-o` | stdout | 出力 JSON パス |
| `--src-lang` | `eng` | 翻訳元(ISO 639-3。`.json` 入力に `src_lang` があればそちらを優先) |
| `--tgt-lang` | `jpn` | 翻訳先(ISO 639-3) |
...
| `--tgt-lang` | `jpn` | 合成言語ヒント(ISO 639-3。`.json` 入力に `tgt_lang` があればそちらを優先) |
...
| `--out-dir` / `-O` | (必須) | 出力ディレクトリ |
| `--src-lang` / `--tgt-lang` | `eng` / `jpn` | 翻訳の言語ペア(ISO 639-3。JSON 入力時は JSON 値を優先) |
...
| `--tgt-lang` | `jpn` | `--text` 指定時に TTS に渡す言語ヒント(ISO 639-3) |
