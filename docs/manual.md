...
# 全部のせ(追加 backend を全部入れる集約プリセット):
# py -m uv sync --extra cpu  --extra full     # CPU 全部のせ
# py -m uv sync --extra cuda --extra full     # CUDA 全部のせ
```

追加 backend(VAD 3 種 / クラウド ASR・翻訳・TTS / Piper / ProcTap 等)は **extras を
入れた分だけ** GUI のプルダウンに現れます(未導入の backend は列挙されません)。
個別に選びたい場合の extras 対応は `pyproject.toml` の
`[project.optional-dependencies]` を参照してください。
...
# 全部のせ環境で動かす(集約プリセットも同様に付ける)
py -m uv run --extra cuda --extra full python -m voice_translator
```
...
プルダウンには**依存パッケージが導入済みの backend だけ**が列挙されます。追加 backend を
使いたい場合は対応する extras を入れて再起動してください(全部入れるなら
`py -m uv sync --extra cpu --extra full`(GPU 環境は `cuda`)。個別の対応表は
