...
各バックエンドの**右側にステータスラベル**が表示されます(英語、色付き):
- **`Not Downloaded`(赤)** — モデルが未DL。「開始」を押すと自動でDLが走ります(NLLB-200は約2.5GBで時間がかかります)
- **`Loading...`(オレンジ)** — DL中 or メモリへロード中
- **`Loaded`(緑)** — メモリにロード済み or キャッシュ済みで即ロード可能
...
## 6. 動作開始/停止
- 「▶ 開始」ボタン押下後、**ボタンは「モデル準備中…」になり**、各レイヤのステータスが Loading → Loaded と順に変化します(UI は固まりません)。
- 全レイヤが Loaded になると **ボタンが「■ 停止」に切り替わり**、パイプラインが動き出します。
