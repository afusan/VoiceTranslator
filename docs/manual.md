...
> (認証情報未設定 / モデル DL 中 / プロセス未選択 のときだけ無効化)。
...
- **翻訳レイヤは使われなくなる**(ロードもされない)。翻訳行のステータス欄は
  「(ASR 側で実行: faster_whisper_translate)」のように**実際に動くバックエンド名**の
  グレー表示になり、プルダウンと設定ボタンは **disabled**(触っても意味がない状態)
  になる。プルダウンの選択値自体は記憶されたままで、ASR を単体バックエンドに戻すと
...
- ボタンが無効化されるのは「認証情報未設定」「モデル DL 中…」「プロセス未選択」のときだけです。
  `Init` / `Loading...` が残っていても押せます(ラベルに「停止中(押下時にロードします)」等の補助表示)。
